...
<style>
  .foto-item{border-radius:10px;overflow:hidden;cursor:pointer;transition:transform .25s,opacity .25s;aspect-ratio:1/1;}
  .foto-item:hover{transform:scale(1.04);opacity:.9;}
  /* Lightbox */
  ...
  #lightbox-close{position:absolute;top:24px;right:32px;font-size:32px;color:#fff;cursor:pointer;font-weight:300;line-height:1;}
  #lightbox-label{position:absolute;bottom:30px;left:50%;transform:translateX(-50%);color:#ddd;font-size:14px;}
...
<main class="mx-auto max-w-[1280px] px-14 py-10 mt-[72px]">
  ...
  <div class="mt-10 flex items-center justify-center gap-2">
    ...
    @foreach ($fotos->getUrlRange(1, $fotos->lastPage()) as $page => $url)
      <a href="{{ $url }}" class="pg-btn {{ $page == $fotos->currentPage() ? 'active' : '' }}">{{ $page }}</a>
    @endforeach
...
<div id="lightbox" onclick="closeLightbox()">
  ...
  <div id="lightbox-inner" style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;">
    ...
    <div id="lightbox-ph" style="width:500px;height:350px;border-radius:16px;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:10px;">
      <svg id="lightbox-icon" style="width:48px;height:48px;color:rgba(255,255,255,0.5);" fill="currentColor" viewBox="0 0 24 24"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/></svg>
